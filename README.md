...
│ 入口     │ 何时用             │ 产物                   │
├──────────┼────────────────────┼────────────────────────┤
│ 普通分析 │ 追求视觉接近原图   │ 原图底图 + 辅助框      │
│ AI重建   │ 需要语义结构       │ 底图 + 语义节点/连线   │
└──────────┴────────────────────┴────────────────────────┘
```
...
│ 模式     │ 特点                       │ 适合场景           │
├──────────┼────────────────────────────┼────────────────────┤
│ 普通分析 │ 保留底图，附加辅助框       │ 快速复刻、手工细修 │
│ AI重建   │ 生成语义节点和连线         │ 二次设计、结构编辑 │
└──────────┴────────────────────────────┴────────────────────┘
```
...
导入、AI 重建和导出都会经过运行时校验。服务端会修复缺失 metadata、重复 id、非法颜色、异常尺寸和悬空连线；无法可靠修复的边会被丢弃。
...
4. 修复 scene 中的 id、样式、尺寸和连线
5. 自动插入锁定的原图底图（ensureReplicaBaseLayer）
...
后端启动时会清理 `data/uploads`、`data/exports`、`data/scenes` 中超过 `DATA_RETENTION_DAYS` 的受管文件。默认保留 14 天。

```powershell
$env:DATA_RETENTION_DAYS = "14"
```

清理不递归删除目录，只处理图片、JSON、SVG、PPTX 等运行产物。
...
│ texts          │ 文本节点数量                 │
│ shapes         │ 非图片、非文本节点数量       │
│ endpointIssues │ 端点引用不存在节点的次数     │
│ meanDiff       │ SVG 与原图的平均像素差       │
└────────────────┴──────────────────────────────┘
```
